...
class Unet(object):
    ...
    def __init__(self, channels=3, n_class=2, cost="cross_entropy", cost_kwargs={}, **kwargs):
        ...
        self.w = tf.placeholder("float", shape=[None, None, None, 1])
        ...
        self.dice = dice_coef(logits, self.y);
    ...
    def _get_cost(self, logits, cost_name, cost_kwargs):
        ...
        #flat_weight = tf.reshape(self.w, [-1, 1])
        ...
        if cost_name == "cross_entropy":
            ...
            if class_weights is not None:
                ...
                #weighted_loss = tf.mul(loss_map, flat_weight)  
                #weighted_loss = tf.multiply(loss_map, flat_weight)      
                #loss = tf.reduce_mean(weighted_loss)
                #loss = tf.reduce_mean(loss_map)

                z = pixel_wise_softmax_2(flat_logits);
                zhat = flat_labels * flat_weight * tf.log( tf.clip_by_value(z,1e-10,1.0) );
                loss = -tf.reduce_mean( zhat  );
    ...
    def predict(self, model_path, x_test):
        ...
        with tf.Session() as sess:
            ...
            w_dummy = np.empty((x_test.shape[0], x_test.shape[1], x_test.shape[2], 1))
...
class Trainer(object):
    ...
    def train(self, data_provider, output_path, training_iters=10, epochs=100, dropout=0.75, display_step=1, restore=False):
        ...
        with tf.Session() as sess:
            ...
            test_x, test_y, test_w = data_provider(self.verification_batch_size)
            ...
            for epoch in range(epochs):
                ...
                for step in range((epoch*training_iters), ((epoch+1)*training_iters)):
                    batch_x, batch_y, batch_w = data_provider(self.batch_size)
                    ...
                    if step % display_step == 0:
                        self.output_minibatch_stats(sess, summary_writer, step, batch_x, util.crop_to_shape(batch_y, pred_shape), util.crop_to_shape(batch_w, pred_shape))
    ...
    def output_minibatch_stats(self, sess, summary_writer, step, batch_x, batch_y, batch_w):
        ...
        summary_str, loss, acc, dc, predictions = sess.run([self.summary_op, 
                                                            self.net.cost, 
                                                            self.net.accuracy, 
                                                            self.net.dice,
                                                            self.net.predicter], 
        ...
        logging.info("Iter {:}, Minibatch Loss= {:.4f}, Training Accuracy= {:.4f}, Minibatch error= {:.1f}%, Dice: {:.4f}".format(step,
                                                                                                            loss,
                                                                                                            acc,
